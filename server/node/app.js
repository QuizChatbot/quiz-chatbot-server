...
app.get('/webhook', (req, res, question) => {
    ...
    switch (messageText) {
      ...
      default: {
        if (state === 0) //user chat with bot for the first time
          ...
          answerForEachQuestion = readJson.getAllAnswerFromQuestion("1")
          if(answerForEachQuestion == null){ 
            console.log("Doesn't have this id in questions json")
            return null
          }

          let buttonsCreated = readJson.createButtonFromQuestionId("1")
...
function nextQuestion(senderID){
  let keyToButton = utillArray.shuffleKeyFromQuestions(questions)
  
  //no question left
  if(keyToButton == null) sendTextMessage(senderID, "Finish!")
  let keyToDelete
  console.log("keyTobutton = ", keyToButton)
    
  //check whether questions contain that key to set button message
  for(let key in questions){
    if(key == keyToButton){
      keyToDelete = key
      console.log("key same = ", key)
    }
  }
  
  console.log("the rest = ", questions)

  //no key that matched question
  answerForEachQuestion = readJson.getAllAnswerFromQuestion(keyToButton)
  if(answerForEachQuestion == null){ 
      console.log("Doesn't have this id in questions json")
      return null
  }
  
  let buttonsCreated = readJson.createButtonFromQuestionId(keyToButton)
  let buttonMessage = readJson.createButtonMessageWithButtons(senderID, buttonsCreated)
  delete questions[keyToDelete] //keyToDelete = keyToButton
  callSendAPI(buttonMessage)    
}
